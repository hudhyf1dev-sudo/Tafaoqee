    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="sticky bottom-6 flex justify-center">
        {(isSubmitted || (section.type === SectionType.MATCHING && isSubmitted)) ? (
          <button 
            onClick={handleNext}
            className="bg-[#0056b3] text-white px-10 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 active:scale-95 transition-all w-full md:w-auto"
          >
            {section.type === SectionType.FILL_GAPS 
              ? (currentGroupIdx === groupedQuestions.length - 1 ? 'إنهاء كافة التمارين ✔' : 'التمرين التالي ➔')
              : (currentIdx === section.questions.length - 1 ? 'إنهاء القسم والنتيجة' : 'السؤال التالي ➔')}
          </button>
        ) : (section.type !== SectionType.GRAMMAR_MCQ && section.type !== SectionType.MATCHING) && (
          <button 
            onClick={() => handleSubmit()}
            className="bg-green-600 text-white px-10 py-4 rounded-2xl font-bold shadow-xl disabled:opacity-50 hover:bg-green-700 transition-all w-full md:w-auto"
          >
            تأكيد حل التمرين ✔
          </button>
        )}
